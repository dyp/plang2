        ...
        const PredicateTypePtr &_pRhs, tc::FormulaList &_formulas)
{
    ...
    for (size_t i = 0; i < _pLhs->getInParams().size(); ++ i) {
        ...
        if (p.getType()->getKind() == Type::TYPE || q.getType()->getKind() == Type::TYPE)
            _formulas.push_back(std::make_shared<tc::Formula>(tc::Formula::EQUALS, p.getType(), q.getType()));
        else
            _formulas.push_back(std::make_shared<tc::Formula>(_kind, q.getType(), p.getType()));
    }
    ...
    for (size_t j = 0; j < _pLhs->getOutParams().size(); ++j) {
        ...
        for (size_t i = 0; i < b.size(); ++ i) {
            ...
            _formulas.push_back(std::make_shared<tc::Formula>(_kind, p.getType(), q.getType()));
        }
    }
        ...
        tc::FormulaList & _formulas, bool _bAllowCompound)
{
    ...
    for (size_t i = 0; i < cOrdFieldsL && i < cOrdFieldsR; ++i) {
        ...
        if (pStrict)
            pStrict->addPart().insert(std::make_shared<tc::Formula>(tc::Formula::SUBTYPE_STRICT,
...
{
    _formulas.push_back(std::make_shared<tc::Formula>(_kind, _pLhs->getBaseType(), _pRhs->getBaseType()));
    return true;
}
...
bool Expand::_expandMap(int _kind, const MapTypePtr &_pLhs, const MapTypePtr &_pRhs,
        tc::FormulaList & _formulas)
{
    _formulas.push_back(std::make_shared<tc::Formula>(_kind, _pRhs->getIndexType(), _pLhs->getIndexType()));
    _formulas.push_back(std::make_shared<tc::Formula>(_kind, _pLhs->getBaseType(), _pRhs->getBaseType()));
    return true;
}
...
bool Expand::_expandArray(int _kind, const ArrayTypePtr &_pLhs, const ArrayTypePtr &_pRhs, tc::FormulaList &_formulas) {
    _formulas.push_back(std::make_shared<tc::Formula>(_kind, _pLhs->getBaseType(), _pRhs->getBaseType()));
    _formulas.push_back(std::make_shared<tc::Formula>(_kind, _pRhs->getDimensionType(), _pLhs->getDimensionType()));
    return true;
}
        ...
        tc::FormulaList & _formulas)
{
    if (_pLhs->getDeclaration() && _pLhs->getDeclaration()->getType() &&
        ...
        _formulas.push_back(std::make_shared<tc::Formula>(_kind, _pLhs->getDeclaration()->getType(),
...
bool Expand::_expandSubtype(int _kind, const SubtypePtr &_pLhs, const SubtypePtr &_pRhs, tc::FormulaList &_formulas) {
    const tc::FreshTypePtr
        pMinType = std::make_shared<tc::FreshType>(tc::FreshType::PARAM_IN);

    const NamedValuePtr
        pParam = std::make_shared<NamedValue>(L"", pMinType);
    ...
    const ExpressionPtr
        pImplication = std::make_shared<Binary>(Binary::IMPLIES,
    ...
    _formulas.push_back(std::make_shared<tc::Formula>(tc::Formula::SUBTYPE,
...
bool Expand::_expandSubtypeWithType(int _kind, const TypePtr &_pLhs, const TypePtr &_pRhs, tc::FormulaList &_formulas) {
    ...
    const auto pSubtype = _pRhs->as<Subtype>();
    const TypePtr& pType = _pLhs;

    Cloner cloner;

    const NamedValuePtr
        pParam = std::make_shared<NamedValue>(L"", pType);
...
bool Expand::_expandUnionType(int _kind, const UnionTypePtr& _pLhs, const UnionTypePtr& _pRhs, tc::FormulaList &_formulas) {
    for (size_t i = 0; i < _pLhs->getConstructors().size(); ++i) {
        ...
        _formulas.push_back(std::make_shared<tc::Formula>(_kind,
