...
void Context::collectPaths(const NodePtr &_node) {
...
bool PrettyPrinterSyntax::traverseModule(const ModulePtr &_module) {
...
// NODE / LABEL
bool PrettyPrinterSyntax::visitLabel(const LabelPtr &_label) {
...
bool PrettyPrinterSyntax::visitType(const TypePtr &_type) {
...
bool PrettyPrinterSyntax::visitTypeType(const TypeTypePtr &_type) {
    m_os << L"type";
    return false;
}

bool PrettyPrinterSyntax::traverseNamedReferenceType(const NamedReferenceTypePtr &_type) {
...
bool PrettyPrinterSyntax::visitSetType(const SetTypePtr &_type) {
...
bool PrettyPrinterSyntax::visitListType(const ListTypePtr &_type) {
...
bool PrettyPrinterSyntax::visitRefType(const RefTypePtr &_type) {
...
bool PrettyPrinterSyntax::traverseMapType(const MapTypePtr &_type) {
...
bool PrettyPrinterSyntax::traverseRange(const RangePtr &_type) {
...
bool PrettyPrinterSyntax::visitArrayType(const ArrayTypePtr &_type) {
    const auto pBaseType = _type->getRootType();
...
bool PrettyPrinterSyntax::traverseEnumType(const EnumTypePtr &_type) {
...
bool PrettyPrinterSyntax::_traverseStructType(const StructTypePtr &_type) {
...
bool PrettyPrinterSyntax::traverseStructType(const StructTypePtr &_type) {
...
bool PrettyPrinterSyntax::traverseUnionConstructorDeclaration(const UnionConstructorDeclarationPtr &_cons) {
    ...
    if (const auto pStruct = _cons->getStructFields()) {
...
bool PrettyPrinterSyntax::traverseUnionType(const UnionTypePtr &_type) {
...
bool PrettyPrinterSyntax::traversePredicateType(const PredicateTypePtr &_type) {
...
bool PrettyPrinterSyntax::traverseParameterizedType(const ParameterizedTypePtr &_type) {
    VISITOR_ENTER(ParameterizedType, _type);
    VISITOR_TRAVERSE_NS(Type, ParameterizedTypeBase, _type->getActualType());
    VISITOR_EXIT();
}

bool PrettyPrinterSyntax::visitSeqType(const SeqTypePtr& _type) {
    assert(false);
    return false;
}

bool PrettyPrinterSyntax::visitOptionalType(const OptionalTypePtr& _type) {
    assert(false);
    return false;
}

bool PrettyPrinterSyntax::traverseJump(const JumpPtr &_stmt) {
...
bool PrettyPrinterSyntax::traverseBlock(const BlockPtr &_stmt) {
    ...
    for (size_t i = 0; i < _stmt->size(); ++i) {
        ...
        m_os << L"\n";
        VISITOR_TRAVERSE_ITEM_NS(Statement, Stmt, *_stmt.get(), i);
    }
...
bool PrettyPrinterSyntax::traverseParallelBlock(const ParallelBlockPtr &_stmt) {
    ...
    VISITOR_TRAVERSE_ITEM_NS(Statement, Stmt, *_stmt.get(), 0);
    m_os << indent;
...
bool PrettyPrinterSyntax::_traverseAnonymousPredicate(const AnonymousPredicatePtr &_decl) {
    m_os << L"(" << setInline();
    ...
    for (size_t i = 0; i < _decl->getOutParams().size(); ++i) {
        ...
        bHavePreConditions |= bool(br.getPreCondition());
        bHavePostConditions |= bool(br.getPostCondition());
    ...
    bHavePreConditions |= bool(_decl->getPreCondition());
    bHavePostConditions |= bool(_decl->getPostCondition());
...
bool PrettyPrinterSyntax::traversePredicate(const PredicatePtr &_stmt) {
...
bool PrettyPrinterSyntax::traverseAssignment(const AssignmentPtr &_stmt) {
...
bool PrettyPrinterSyntax::traverseMultiassignment(const MultiassignmentPtr &_stmt) {
...
VariableDeclarationPtr getVarDecl(const CallPtr& _call, const ExpressionPtr& _pArg) {
    if (!_pArg || _pArg->getKind() != Expression::VAR)
        return NULL;
    for (size_t i = 0; i < _call->getDeclarations().size(); ++i) {
        const auto pVar = _call->getDeclarations().get(i);
...
bool PrettyPrinterSyntax::traverseCall(const CallPtr &_stmt) {
...
bool PrettyPrinterSyntax::traverseIf(const IfPtr &_stmt) {
...
bool PrettyPrinterSyntax::traverseSwitchCase(const SwitchCasePtr &_case) {
...
bool PrettyPrinterSyntax::traverseSwitch(const SwitchPtr &_stmt) {
    ...
    m_os << L") {";
    VISITOR_TRAVERSE_COL(SwitchCase, SwitchCase, *_stmt.get());
...
bool PrettyPrinterSyntax::traverseFor(const ForPtr &_stmt) {
...
bool PrettyPrinterSyntax::traverseWhile(const WhilePtr &_stmt) {
...
bool PrettyPrinterSyntax::traverseBreak(const BreakPtr &_stmt) {
...
bool PrettyPrinterSyntax::traverseWith(const WithPtr &_stmt) {
...
bool PrettyPrinterSyntax::traverseTypeDeclaration(const TypeDeclarationPtr &_stmt) {
...
bool PrettyPrinterSyntax::traverseVariableDeclaration(const VariableDeclarationPtr &_stmt) {
...
bool PrettyPrinterSyntax::traverseVariableDeclarationGroup(const VariableDeclarationGroupPtr &_stmt) {
    VISITOR_ENTER(VariableDeclarationGroup, _stmt);
    VISITOR_TRAVERSE_NS(Label, StmtLabel, _stmt->getLabel());
    VISITOR_TRAVERSE_COL(VariableDeclaration, VarDeclGroupElement, *_stmt.get());
    VISITOR_EXIT();
}

bool PrettyPrinterSyntax::traverseFormulaDeclaration(const FormulaDeclarationPtr &_node) {
...
bool PrettyPrinterSyntax::traverseLemmaDeclaration(const LemmaDeclarationPtr &_stmt) {
...
// NODE / NAMED_VALUE
bool PrettyPrinterSyntax::traverseNamedValue(const NamedValuePtr &_val) {
...
bool PrettyPrinterSyntax::traverseEnumValue(const EnumValuePtr &_val) {
    VISITOR_ENTER(EnumValue, _val);
    m_os << m_pContext->nameGenerator().getNamedValueName(_val);
    VISITOR_EXIT();
}

bool PrettyPrinterSyntax::traverseParam(const ParamPtr &_val) {
...
bool PrettyPrinterSyntax::traverseVariable(const VariablePtr &_val) {
...
// NODE / EXPRESSION
void PrettyPrinterSyntax::printLiteralKind(const LiteralPtr &_node) {
...
void PrettyPrinterSyntax::printUnaryOperator(const UnaryPtr &_node) {
...
void PrettyPrinterSyntax::printBinaryOperator(const BinaryPtr &_node) {
    switch (_node->getOperator()) {
        case Binary::ADD:
            m_os << L" + ";
            break;
...
bool PrettyPrinterSyntax::needsParen() {
    ...
    const auto pNode = getLoc().pNode->as<Expression>();
    ...
    switch (getRole()) {
        ...
        case R_BinarySubexpression:
            ...
            return (Binary::getPrecedence(pNode->as<Binary>()->getOperator()) <
                    Binary::getPrecedence(getParent()->as<Binary>()->getOperator()));
...
bool PrettyPrinterSyntax::traverseExpression(const ExpressionPtr &_node) {
...
bool PrettyPrinterSyntax::visitLiteral(const LiteralPtr &_node) {
    printLiteralKind(_node);
    return true;
}

bool PrettyPrinterSyntax::visitVariableReference(const VariableReferencePtr &_node) {
...
bool PrettyPrinterSyntax::visitPredicateReference(const PredicateReferencePtr &_node) {
    m_os << _node->getName();
    return true;
}

bool PrettyPrinterSyntax::visitLambda(const LambdaPtr &_node) {
    m_os << L"predicate";
    return true;
}

bool PrettyPrinterSyntax::traverseBinder(const BinderPtr &_expr) {
...
bool PrettyPrinterSyntax::visitUnary(const UnaryPtr &_node) {
    printUnaryOperator(_node);
    return true;
}

bool PrettyPrinterSyntax::traverseBinary(const BinaryPtr &_node) {
...
bool PrettyPrinterSyntax::traverseTernary(const TernaryPtr &_node) {
...
bool PrettyPrinterSyntax::traverseFormula(const FormulaPtr &_node) {
...
bool PrettyPrinterSyntax::traverseReplacement(const ReplacementPtr &_expr) {
...
bool PrettyPrinterSyntax::traverseRecognizerExpr(const RecognizerExprPtr &_expr) {
...
bool PrettyPrinterSyntax::traverseAccessorExpr(const AccessorExprPtr &_expr) {
...
bool PrettyPrinterSyntax::traverseFunctionCall(const FunctionCallPtr &_expr) {
...
bool PrettyPrinterSyntax::traverseFormulaCall(const FormulaCallPtr &_expr) {
...
bool PrettyPrinterSyntax::traverseStructFieldDefinition(const StructFieldDefinitionPtr &_cons) {
...
bool PrettyPrinterSyntax::traverseStructConstructor(const StructConstructorPtr &_expr) {
    VISITOR_ENTER(StructConstructor, _expr);
    m_os << L"(" << indent;
    VISITOR_TRAVERSE_COL(StructFieldDefinition, StructFieldDef, *_expr.get());
    m_os << unindent << L")";
    VISITOR_EXIT();
}

bool PrettyPrinterSyntax::traverseUnionConstructor(const UnionConstructorPtr &_expr) {
    VISITOR_ENTER(UnionConstructor, _expr);
    m_os << _expr->getName() << L"(" << indent;
    VISITOR_TRAVERSE_COL(StructFieldDefinition, UnionCostructorParam, *_expr.get());
    m_os << unindent << L")";
    VISITOR_EXIT();
}

bool PrettyPrinterSyntax::traverseElementDefinition(const ElementDefinitionPtr &_cons) {
...
bool PrettyPrinterSyntax::traverseArrayConstructor(const ArrayConstructorPtr &_expr) {
    VISITOR_ENTER(ArrayConstructor, _expr);
    m_os << L"[" << indent;
    VISITOR_TRAVERSE_COL(ElementDefinition, ArrayElementDef, *_expr.get());
    m_os << unindent << L"]";
    VISITOR_EXIT();
}

bool PrettyPrinterSyntax::traverseMapConstructor(const MapConstructorPtr &_expr) {
    VISITOR_ENTER(MapConstructor, _expr);
    m_os << L"[{" << indent;
    VISITOR_TRAVERSE_COL(ElementDefinition, MapElementDef, *_expr.get());
    m_os << unindent << L"}]";
    VISITOR_EXIT();
}

bool PrettyPrinterSyntax::traverseSetConstructor(const SetConstructorPtr &_expr) {
    VISITOR_ENTER(SetConstructor, _expr);
    m_os << L"{" << indent;
    VISITOR_TRAVERSE_COL(Expression, SetElementDef, *_expr.get());
    m_os << unindent << L"}";
    VISITOR_EXIT();
}

bool PrettyPrinterSyntax::traverseListConstructor(const ListConstructorPtr &_expr) {
    VISITOR_ENTER(ListConstructor, _expr);
    m_os << L"[[" << indent;
    VISITOR_TRAVERSE_COL(Expression, ListElementDef, *_expr.get());
    m_os << unindent << L"]]";
    VISITOR_EXIT();
}

bool PrettyPrinterSyntax::traverseArrayPartDefinition(const ArrayPartDefinitionPtr &_cons) {
...
bool PrettyPrinterSyntax::traverseArrayIteration(const ArrayIterationPtr &_expr) {
    VISITOR_ENTER(ArrayIteration, _expr);

    m_os << L"for (" << indent;
    VISITOR_TRAVERSE_COL(NamedValue, ArrayIterator, _expr->getIterators());
    m_os << L") {";
    VISITOR_TRAVERSE_COL(ArrayPartDefinition, ArrayIterationPart, *_expr.get());
...
bool PrettyPrinterSyntax::traverseArrayPartExpr(const ArrayPartExprPtr &_expr) {
...
bool PrettyPrinterSyntax::traverseFieldExpr(const FieldExprPtr &_expr) {
...
bool PrettyPrinterSyntax::traverseMapElementExpr(const MapElementExprPtr &_expr) {
...
bool PrettyPrinterSyntax::traverseListElementExpr(const ListElementExprPtr &_expr) {
...
bool PrettyPrinterSyntax::traverseCastExpr(const ir::CastExprPtr &_expr) {
...
bool PrettyPrinterSyntax::visitConstructor(const ConstructorPtr& _expr) {
...
void PrettyPrinterSyntax::print(const NodePtr &_pNode) {
    ...
    m_pContext->nameGenerator().collect(_pNode);
    m_pContext->collectPaths(_pNode);
    traverseNode(_pNode);
}

void prettyPrintSyntax(const NodePtr &_node, std::wostream & _os, const ContextPtr& _pContext, bool _bNewLine) {
    PrettyPrinterSyntax(_node, _os, _pContext).run();
    if (_bNewLine)
        _os << L"\n";
}

void prettyPrintSyntax(const NodePtr &_node, size_t nDepth, std::wostream & _os) {
