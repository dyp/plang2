    ...
    virtual bool visitCall(Call &_node) {
                    ...
                    m_recursivePredicates.end())
        {
            ...
            if (!(pParent->pNode->getNodeKind() == Node::STATEMENT &&
               ((Statement *)pParent->pNode)->getKind() == Statement::BLOCK))
            {
                BlockPtr pEnvelope = new Block();
                pEnvelope->add(&_node);
                callSetter(pEnvelope);
                m_blocks.insert(pEnvelope);
            } else
                m_blocks.insert((Statement *)pParent->pNode);
...
// Set new destination for jumps (is used for predicates' blocks).
class JumpAlteration : public Visitor {
public:
    // m_pNewDestionation is new destination for jumps.
...
// Retarget variables (is used for retargeting predicates' parameters to their new declarations).
    ...
    for (const auto &i: _predicateOrder)
        if (i.getPredicates().size() > 1)
            for (const auto &j: i.getPredicates())
                _recursivePredicates.insert(j.first);
        else if (i.isRecursive())
            _recursivePredicates.insert(i.getPredicate());
}
...
static LabelPtr _generatePostCallLabel(const size_t _cCallPosition, const BlockPtr &_pBlock) {
    ...
    if (_cCallPosition < _pBlock->size() - 1 && _pBlock->get(_cCallPosition + 1)->getLabel())
        ...
        if (_cCallPosition < _pBlock->size() - 1)
            ...
            const StatementPtr pStmt = new Statement(pPostJumpLabel);
        ...
        const BlockPtr &_pBlock, const BlockPtr &_pInsertBlock)
{
    ...
    for (size_t k = 0; k < _pCalledPred->getOutParams().size(); ++k) {
        ...
        if (pBranchStmt) {
            JumpPtr pPostJump = new Jump(pPostJumpLabel);
...
static void _predResultsAlteration(const CallPtr &_pCall, PredicatePtr &_pCalledPred) {
...
void ir::predicateInlining(Module &_module) {
    CallGraph graph;
    ...
    for (const auto &i: predicateOrder)
        for (const auto &j: i.getPredicates()) {
            ...
            callSearch.traverseNode(*j.first);
            ...
            for (const auto &pBlock: blocks)
                for (size_t k = 0; k != pBlock->size(); ++k) {
                    ...
                    const CallPtr pCall = pBlock->get(k).as<Call>();
                    PredicatePtr pCalledPred = clone(pCall->getPredicate().as<PredicateReference>()->getTarget());
                    BlockPtr pInsertBlock = new Block();
                    ...
                    for (size_t l = 0; l != pCalledPred->getInParams().size(); ++l) {
                        VariablePtr pVar = new Variable(true, L"", pCalledPred->getInParams().get(l)->getType());
                        VariableDeclarationPtr pDecl = new VariableDeclaration(pVar, pCall->getArgs().get(l));
                    ...
                    // Retarget arguments inside predicate's body to these new declarations.
