...
bool _handlePrettyPrint(const std::string &_val, void *_p) {
    ...
    else if (_val == "cg" || _val == "5")
        opts.prettyPrint |= PP_CALLGRAPH;
    else
        return false;
...
void _printUsage() {
    std::cerr << "Usage:\n\n"
        << "    plang [OPTIONS] FILE\n\n"
        << "Options\n\n"
        << "    -p, --prettyprint=TYPE        Pretty-printer mode, where TYPE is 'none' (0), 'flat' (1), 'ast' (2), 'lex' (3), 'p' (4), 'cg' (5)\n"
...
bool Options::init(size_t _cArgs, const char **_pArgs) {
    ...
    Option options[] = {
        ...
        { "type-info",       'i', NULL,               &instance().bTypeInfo,       NULL,                          false },
